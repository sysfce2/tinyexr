 ...
 * an ephemeral set of C11 (<threads.h>) workers that pull jobs from a shared,
 * mutex-protected counter; the calling thread participates too. When disabled
 * (the default, and always for freestanding builds) it runs the jobs serially
 * and pulls in no threading headers.
 *
