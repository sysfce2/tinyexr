        ...
        switch (op->kind) {
            ...
            case TOC_OP_FIXEDFUNC: {
                ...
                if (s == TOC_FF_REC2100_SURROUND ||
                    ...
                    emit_f(&sb, e);
                    toc_sb_puts(&sb, "); }\n");
                } else if (s == TOC_FF_ACES_RED_MOD_03 || s == TOC_FF_ACES_RED_MOD_10) {
                    float hw = (s == TOC_FF_ACES_RED_MOD_03) ? 1.909859f : 1.697653f;
                    int is03 = (s == TOC_FF_ACES_RED_MOD_03);
                    toc_sb_puts(&sb, "  { float3 c=v.rgb;float a=2.0*c.r-(c.g+c.b);float b=1.73205*(c.g-c.b);\n"
                                     "  float h=atan2(b,a);float k=h*"); emit_f(&sb, hw);
                    toc_sb_puts(&sb, "+2.0;int j=int(k);\n"
                    ...
                    if (is03)
                        toc_sb_puts(&sb, "  float nr=c.r+fH*fS*(0.03-c.r)*0.15;\n"
                                         "  if(c.g>=c.b)c.g=(c.g-c.b)/max(1e-10,c.r-c.b)*(nr-c.b)+c.b;\n"
                                         "  else c.b=(c.b-c.g)/max(1e-10,c.r-c.g)*(nr-c.g)+c.g;\n"
                                         "  c.r=nr;}v.rgb=c;} }\n");
                    else
                        toc_sb_puts(&sb, "  c.r=c.r+fH*fS*(0.03-c.r)*0.18;}v.rgb=c;} }\n");
